...
def test_agg_df():
    ...
    expected_output_matrix =  [(1, 34.0, 'Cardiology'),
                     (2, 33.0, 'Neurology'),  # 33.0 est la moyenne des âges
                     (3, 50.0, 'Orthopedics'),
                     (4, 33.0, 'Unknown'),
                     (5, 15.0, 'Neurology')]
    ...
    age_mean = input_df.select(F.avg(F.col('age'))).first()[0]
    df = input_df.fillna({'age': age_mean, 'department': 'Unknown'})
